...
def CityWeather(city,type):
    ...
    query = 'SELECT DISTINCT ?name ?lon ?lat WHERE { ?target ns:city ?city . ?target ns:city "cityname" . ?target ns:longitude ?lon . ?target ns:latitude ?lat. }'
    q = query.replace("cityname",city)
    ...
    query='SELECT DISTINCT ?name ?lon ?lat WHERE { ?target ns:city ?city . ?name ns:city ?city . ?name ns:longitude ?lon . ?name ns:latitude ?lat. }'
    print(query)
    res = g.query(query)
    print("hmm")
    print(res)
